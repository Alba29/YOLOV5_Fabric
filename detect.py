...
"""
    ...
    save-txt: 是否将预测的框坐标以txt文件格式保存  会在runs/detect/expn/labels下生成每张图片预测的txt文件
    save-conf: 是否保存预测每个目标的置信度到预测txt文件中 
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    if save_txt:
        import csv
        with open(str(save_dir / 'results.csv'), 'a', newline='') as csvf:
            writer = csv.writer(csvf)
            writer.writerow(["ImageID", "class", "confidence", "xmid", "ymid", "width", "height"])
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        # debug flag
                        with open(str(save_dir / 'results.csv'), 'a') as csvf:
                            writer = csv.writer(csvf)
                            writer.writerow([p.name, cls.item(), conf.item(), *xywh])                       
